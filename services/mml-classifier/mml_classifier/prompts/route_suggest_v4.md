...
**Two new signals available to you in v4 (read them carefully):**

1. **Email body content** — the user message now includes the body of the email (truncated to ~2000 chars), not just sender + subject. Read it. The body often disambiguates an unclear subject — a generic-sounding "Update from XYZ Co." might be a newsletter (`fun`), a sales pitch (`coach sales` or `deals`), a notification (`extra`), etc., and the body tells you which.
