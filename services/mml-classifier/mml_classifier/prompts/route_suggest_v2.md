...
- the owner would rather see a confident suggestion he overrides than a hesitant `none`. But DO return `none` for the NYT exception and for genuinely off-axis mail (e.g., a personal email from a friend, a one-off business contract — not promotional, not noise).
- Sender-domain heuristics (high-confidence shortcuts):
  - `substack.com` → `fun` (unless overtly political → `pol`)
  - Major retailers, consumer brands, marketing automation domains (`mailchimp.com`, `klaviyomail.com`, `cmail*.com` envelopes from retailers) → `deals`
  - Social platforms (`facebookmail.com`, `linkedin.com`, `twitter.com`, `x.com`, `instagram.com`, `bsky.social`, `threads.net`, `tiktok.com`) → `smm`
  - WordPress / hosting / SaaS-notification domains (`wordpress.com`, `*.wpengine.com`, `notifications@*` platform domains with low-content subjects) → `extra`
...
- `confidence` — float in [0.0, 1.0]. Use 0.9+ only when sender domain alone makes it obvious (e.g. `noreply@facebookmail.com` → `smm`). Subject-line-based calls should be 0.4–0.7. Heuristic-rule matches (Substack, NYT, retailers, social platforms) warrant 0.85+.
