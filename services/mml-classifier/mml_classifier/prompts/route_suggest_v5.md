...
DISTINGUISH FROM `Routed/extra`: automated AI-vendor transactional mail (invoices, usage alerts, password resets from `openai.com` / `anthropic.com`) → `extra`. Substantive content from AI vendors (model releases, technical blog posts, research summaries) → `Routed/AI`.
...
DISTINGUISH FROM `Routed/Finance`: `extra` is high-volume technical noise the user doesn't need to keep. `Finance` is a money record the owner may want to retrieve later (a receipt, an invoice, a payment confirmation with a dollar amount).
...
DISTINGUISH FROM `Routed/extra`: `extra` is high-volume technical noise (WordPress signups, generic platform notifications). `Finance` is a money record the owner may want to retrieve.
...
- `suggested_folder` — one of the eleven literal strings above (`"Routed/AI"`, `"Routed/aol7"`, `"Routed/coach sales"`, `"Routed/deals"`, `"Routed/entertaining"`, `"Routed/extra"`, `"Routed/Finance"`, `"Routed/models"`, `"Routed/pol"`, `"Routed/smm"`, `"Routed/Wellness"`) OR the literal string `"none"`.
